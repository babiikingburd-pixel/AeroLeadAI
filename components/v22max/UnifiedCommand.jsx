    ...
    <section style={{maxWidth:1500,margin:"0 auto",padding:"18px 18px 0"}}>
      <div style={{border:`1px solid ${C.cyan}`,borderRadius:12,padding:14,background:"linear-gradient(135deg,#071119ee,#0b1721ee)",boxShadow:"0 0 50px #2ef0ea12"}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"flex-start",flexWrap:"wrap"}}>
          <div><div style={{font:"10px monospace",letterSpacing:2,color:C.cyan}}>APEX16 GATEKEEPER / SYSTEM CONTROL PLANE</div><h2 style={{margin:"5px 0 4px",fontSize:"clamp(22px,4vw,34px)"}}>V22 MAX intelligence under evidence-first supervision</h2><div style={{color:C.muted,maxWidth:900}}>SOURCE → EVIDENCE → FRESHNESS → CORROBORATION → CONTRADICTION → CONFIDENCE → ACTION. Unknown data stays unknown; questionable properties are held instead of promoted as verified.</div></div>
          <div style={{display:"flex",gap:7,flexWrap:"wrap"}}><Stat label="GK VERSION" value={gk?.gatekeeper||"16.0"} tone={C.cyan}/><Stat label="ACTIONABLE" value={gk?.verifiedActionable} tone={C.green}/><Stat label="CAUTION" value={gk?.verifiedWithCaution} tone={C.amber}/><Stat label="HELD" value={gk?.held} tone={C.red}/><Stat label="AVG CONF" value={gk?`${Math.round((gk.averageConfidence||0)*100)}%`:"—"}/></div>
        </div>
        <div style={{marginTop:10,font:"10px monospace",color:err?C.red:(gk?.dataMode==="live"?C.green:C.amber)}}>{err?`GATEKEEPER STATUS ERROR: ${err}`:`${String(gk?.dataMode||"checking").toUpperCase()} · ${gk?.table||"resolving data source"} · sampled ${gk?.sample??"—"} properties`}</div>
      </div>
    </section>
