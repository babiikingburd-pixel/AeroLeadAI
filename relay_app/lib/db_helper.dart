...
class DBHelper {
  ...
  // Added for memory_store.dart / workspace.dart migration off
  // SharedPreferences -- these read/write tables the schema below
  // already defines but that had no Dart methods yet. ------------------

  static Future<List<Map<String, dynamic>>> getWorkspaces() async {
    final db = await database;
    return db.query('workspaces', orderBy: 'created_at ASC');
  }

  static Future<String?> getCurrentFocus(String workspaceId) async {
    final db = await database;
    final rows = await db.query(
      'current_focus',
      where: 'workspace_id = ?',
    ...
    return rows.first['focus_text'] as String?;
  }

  static Future<void> setCurrentFocus(String workspaceId, String focusText) async {
    final db = await database;
    await db.insert(
      'current_focus',
      {
  ...
  /// Projects are stored as {name, details: jsonEncode({status, notes})}.
  static Future<List<Map<String, dynamic>>> getProjects(String workspaceId) async {
    final db = await database;
    final rows = await db.query(
      'project_memory',
      where: 'workspace_id = ?',
    ...
    return rows.map((r) {
      final details = jsonDecode((r['details'] as String?) ?? '{}') as Map<String, dynamic>;
      return {
        'name': r['project_name'],
  ...
  /// Full replace (delete + reinsert) to match the old SharedPreferences
  /// "save the whole list" behavior.
  static Future<void> replaceProjects(
      String workspaceId, List<Map<String, String>> projects) async {
    final db = await database;
    final now = DateTime.now().millisecondsSinceEpoch ~/ 1000;
    await db.transaction((txn) async {
      await txn.delete('project_memory', where: 'workspace_id = ?', whereArgs: [workspaceId]);
      for (final p in projects) {
        await txn.insert('project_memory', {
          'workspace_id': workspaceId,
          'project_name': p['name'],
          'details': jsonEncode({'status': p['status'] ?? 'active', 'notes': p['notes'] ?? ''}),
          'created_at': now,
          'updated_at': now,
        });
      }
  ...
  static Future<List<String>> getGoals(String workspaceId) async {
    final db = await database;
    final rows = await db.query(
      'goals',
      where: 'workspace_id = ?',
  ...
  /// Full replace (delete + reinsert) to match the old SharedPreferences
  /// "save the whole list" behavior.
  static Future<void> replaceGoals(String workspaceId, List<String> goals) async {
    final db = await database;
    final now = DateTime.now().millisecondsSinceEpoch ~/ 1000;
    await db.transaction((txn) async {
      await txn.delete('goals', where: 'workspace_id = ?', whereArgs: [workspaceId]);
