-- Static imagery normalization is metadata-only and does not change a score.
-- Disable only the two named evidence triggers for the short transaction, then
-- re-enable them before repairing the affected Doctor state set-wise.
...
begin
  ...
  alter table public.evidence_records disable trigger oversight_superb_score_evidence_trigger;
  ...
  alter table public.evidence_records enable trigger oversight_superb_score_evidence_trigger;
