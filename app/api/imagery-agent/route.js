...
export async function POST(req) {
  ...
  const tightUrl = googleKey
    ...
    : esriExportUrl(lat, lon, 65); // ~130m-wide frame, tight parcel crop
    // (Esri's export service 500s below roughly a 60m half-width at 640px —
    // "Error: bytes" — so this stays a bit above that floor.)
  ...
  try {
    // Esri's free public export service intermittently 500s ("Error: bytes")
    // on the same request that succeeds moments later — genuine upstream
    // flakiness, not just a too-tight bbox — so give it more retries than
    // Google/Mapbox need.
    const overviewRetries = overviewProvider === "esri" ? 3 : 1;
    const tight = await fetchAsDataUrl(tightUrl, overviewRetries);
    const context = await fetchAsDataUrl(contextUrl, overviewRetries);
    if (tight) {
    ...
    if (context) {
      result.angles.overview_context = context;
      if (!result.dataUrl) {
        // Tight crop never came through after retries — fall back to the
        // wider context shot as the primary image rather than shipping no
        // imagery at all.
        result.dataUrl = context;
        result.provider = overviewProvider;
        result.notes.push("Using context-zoom shot as primary — tight crop unavailable.");
      }
    }
    if (hybridUrl) {
