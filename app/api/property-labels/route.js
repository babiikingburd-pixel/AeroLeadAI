...
import { DAMAGE_CLASSES, VERDICTS } from "../../../lib/inspect/labels";
...
function normalizeDocument(body) {
  ...
  const verdict = body.verdict == null || body.verdict === "" ? null : VERDICT_IDS.has(body.verdict) ? body.verdict : undefined;
...
export async function GET(request) {
  ...
  const result = await db.from("oversight_property_labels").select("document").eq("property_id", ids.propertyId).eq("shot_key", ids.shotKey).maybeSingle();
  if (result.error) return errorResponse(result.error.message, 502);
  return NextResponse.json({ ok: true, doc: result.data?.document || null }, { headers: { "Cache-Control": "no-store" } });
}
...
export async function POST(request) {
  ...
  const db = supabaseServer();
  if (!db) return errorResponse("Supabase is not configured.", 503);
  const result = await db.from("oversight_property_labels").upsert({
  ...
  }, { onConflict: "property_id,shot_key" }).select("document").single();
  if (result.error) return errorResponse(result.error.message, 502);
