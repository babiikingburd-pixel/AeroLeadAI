...
import { fuseEvidence, recordFusion, FUSION_VERSION } from "../../../../lib/twincities/evidenceFusion";
...
export async function POST(req) {
  ...
  for(const row of rows||[]) {
    ...
    const {error: upErr}=await supabase.from("batch_leads").update({
      ...
      evidence_fusion_version:FUSION_VERSION
    }).eq("id",row.id);
    if(upErr) continue;
    await recordFusion(supabase,row,result);
    out.push({id:row.id,...result});
  }
  return Response.json({ok:true,version:FUSION_VERSION,processed:out.length,results:out});
}
