...
function lastTouched(row) {
  const values = [row.evidence_cycle_at, row.top500_last_investigated_at, row.weather_checked_at, row.permit_checked_at, row.image_fetched_at]
    .map(v => v ? Date.parse(v) : 0).filter(Number.isFinite);
...
function selectSwarm(rankedRows, limit) {
  ...
  const add = row => { if (row && !used.has(String(row.id)) && selected.length < limit) { used.add(String(row.id)); selected.push(row); } };
  ...
  for (const band of BAND_PLAN) {
    ...
    const pool = rows.filter(r => r.__rank >= band.start && r.__rank <= band.end && !used.has(String(r.id)))
      .sort((a,b)=>(a.__lastTouched-b.__lastTouched)||(a.__rank-b.__rank));
    for (const row of pool) { if (slots-- <= 0 || selected.length >= limit) break; add(row); }
  }
  for (const row of rows.filter(r=>!used.has(String(r.id))).sort((a,b)=>(a.__lastTouched-b.__lastTouched)||(a.__rank-b.__rank))) add(row);
...
export async function POST(req) {
  ...
  const { data:ranked, error } = await supabase.from("batch_leads").select("*")
    .eq("sales_status","new").neq("review_status","rejected").gt("priority_score",0)
    .not("lat","is",null).not("lon","is",null)
    .order("priority_score",{ascending:false}).order("confidence_score",{ascending:false,nullsFirst:false}).limit(TOP_POOL);
  ...
  const rows = selectSwarm(ranked, limit), results = [];
  for (const row of rows) {
    ...
    const [permit, weather, imagery] = await Promise.all([
      ...
      fetchJson(`${origin}/api/weather-agent`, { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({lat:row.lat,lon:row.lon,address}) }, 10000),
      fetchJson(`${origin}/api/imagery-agent`, { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({lat:row.lat,lon:row.lon,address:row.address,leadId:row.id,lite:true,force:true}) }, 15000),
    ]);
    ...
    const patch = {
      ...
      permit_notes: JSON.stringify({ checked_at:started, total_permits:records.length, roof_permits:roofPermits.length, source:permit.data?.inDirectory?"directory":"external_or_lookup", swarm_rank:row.__rank }),
      ...
      storm_checked_at: weather.ok ? started : row.storm_checked_at || null,
      weather_evidence: weather.ok ? weather.data : row.weather_evidence || null,
      evidence_cycle_at: started,
      evidence_cycle_version: "APEX14.3-TOP600-SWARM",
    ...
    const { error:updateError } = await supabase.from("batch_leads").update(patch).eq("id", row.id);
    if (updateError) {
      results.push({ id:row.id, rank:row.__rank, address:row.address, persisted:false, error:updateError.message });
      continue;
    }
    ...
    results.push({ id:row.id, rank:row.__rank, address:row.address, persisted:true, scoreBefore:row.priority_score, confidenceBefore:row.confidence_score, permitsFound:records.length, roofPermits:roofPermits.length, weatherChecked:weather.ok, imageryFetched:imagery.ok, rescoreTriggered:rescore.ok });
  }

  const persisted = results.filter(r=>r.persisted).length;
  return Response.json({ ok:persisted>0, version:"APEX14.3-TOP600-SWARM", processed:results.length, persisted, poolSize:ranked.length, bands:BAND_PLAN, results, note:"Real permit, storm/weather, assessor and imagery evidence now persists before live rescoring." });
}
