...
import { DAMAGE_CLASSES, VERDICTS, emptyLabelDoc, toYoloLines } from "../../../lib/inspect/labels";
...
export default function PropertyIntelligence({ property, onClose }) {
  ...
  const fetchImagery = useCallback(async () => {
    if (property.lat == null || property.lon == null) {
      if (property.imagery?.url) {
        setImagery({
          ok: true,
          shots: { overview_tight: { url: property.imagery.url, source: property.imagery.source || "esri-free" } },
          providerNote: "Using cached card imagery — no lat/lon for Street View",
        });
        setActiveShot("overview_tight");
        return;
      }
    ...
    try {
      const res = await fetch(
        `/api/imagery-agent?lat=${property.lat}&lon=${property.lon}&id=${encodeURIComponent(property.id)}`,
        { cache: "no-store" }
      );
      ...
      setImagery(data);
      const firstKey =
        OVERVIEW_ANGLE_ORDER.find((k) => data.shots?.[k]?.url) ||
        Object.keys(data.shots || {}).find((k) => data.shots[k]?.url);
  ...
  }, [property.id, property.lat, property.lon, property.imagery]);

  useEffect(() => {
    fetchImagery();
    fetch(`/api/property-labels?propertyId=${encodeURIComponent(property.id)}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.ok && d.doc) setDoc(d.doc);
      })
      .catch(() => {});
  }, [fetchImagery, property.id]);
  ...
  useEffect(() => {
    setZoom(1);
    setPan({ x: 0, y: 0 });
    setDoc((d) => ({ ...d, shotKey: activeShot, source: active?.source || null }));
  }, [activeShot, active?.source]);
  ...
  function downloadLabels() {
    const img = imgRef.current;
    const blob = new Blob(
      [JSON.stringify({ ...doc, yolo: img ? toYoloLines(doc, img.naturalWidth, img.naturalHeight) : "" }, null, 2)],
    ...
    <div className="intel-overlay" role="dialog" aria-modal="true">
      <div className="intel-panel">
        ...
        <div className="intel-body">
          <div className="inspect-stage" onWheel={wheel}>
            ...
            {active?.url && (
              ...
              >
                <img ref={imgRef} src={active.url} alt={shotLabel(activeShot)} draggable={false} />
                <Boxes
                  boxes={doc.boxes}
                  live={drag}
                  img={imgRef.current}
          ...
          <aside className="intel-side">
              ...
              <div className="shot-strip">
                {shotKeys.map((key) => {
                  ...
                  return (
                    ...
                    >
                      {s.url ? <img src={s.url} alt="" /> : <span>—</span>}
                      <em>{shotLabel(key)}</em>
            ...
            <div className="side-block">
              <h3>Inspect</h3>
              <div className="row">
                <button className={mode === "box" ? "on" : ""} onClick={() => setMode("box")}>Draw box</button>
                ...
                {doc.boxes.map((b) => {
                  ...
                  return (
                    <li key={b.id}>
                      ...
                      {meta?.label}
                      <button onClick={() => setDoc((d) => ({ ...d, boxes: d.boxes.filter((x) => x.id !== b.id) }))}>×</button>
                    </li>
                  );
                })}
                {doc.boxes.length === 0 && <li className="hint">No boxes yet. Drag on the image.</li>}
              </ul>
            </div>
            <div className="side-block">
              <h3>Verdict</h3>
              <div className="row">
                {VERDICTS.map((v) => (
                  <button key={v.id} className={doc.verdict === v.id ? "on" : ""} onClick={() => setDoc((d) => ({ ...d, verdict: v.id }))}>
                    {v.label}
                  </button>
                ))}
              </div>
              <textarea rows={3} placeholder="Notes for training / sales" value={doc.notes} onChange={(e) => setDoc((d) => ({ ...d, notes: e.target.value }))} />
              <div className="row">
                <button className="primary" onClick={save}>{saveState === "saving" ? "Saving…" : saveState === "saved" ? "Saved" : "Save labels"}</button>
                <button onClick={downloadLabels}>Export JSON</button>
              </div>
              <p className="hint">Hail bruises need close-up / drone. If you cannot see the defect, mark Needs drone.</p>
            </div>
          </aside>
        </div>
      </div>
