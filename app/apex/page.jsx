...
export default function ApexPage() {
  ...
  return <main style={{padding:24,color:'#d8f6ff',fontFamily:'JetBrains Mono, monospace',maxWidth:1200,margin:'0 auto'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'end',marginBottom:22}}>
      <div><div style={{fontSize:11,color:'#62d9ff',letterSpacing:3}}>AEROLEADAI / RELEASE TRAIN</div><h1 style={{margin:'8px 0',fontSize:30}}>APEX {APEX_VERSION}</h1><div style={{color:'#7e9aa5'}}>Readable, buildable, installable progression from 9.0 → 9.6.</div></div>
      <div style={{...card,minWidth:190}}><div style={{fontSize:10,color:'#7e9aa5'}}>LIVE RELEASE</div><strong style={{fontSize:24}}>9.6</strong><div style={{color:'#6dffb0'}}>READY TO PACKAGE</div></div>
    </div>
    <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:12,marginBottom:20}}>
      {APEX_RELEASES.map(r=><div key={r.version} style={{...card,borderColor:r.version==='9.6'?'#62d9ff':'#1c3540'}}><div style={{display:'flex',justifyContent:'space-between'}}><strong>APEX {r.version}</strong><span style={{fontSize:10,color:r.version==='9.6'?'#6dffb0':'#62d9ff'}}>{r.status.toUpperCase()}</span></div><h3 style={{margin:'10px 0 6px'}}>{r.name}</h3><ul style={{paddingLeft:18,margin:0,lineHeight:1.7,fontSize:12}}>{r.capabilities.map(x=><li key={x}>{x}</li>)}</ul></div>)}
    </section>
