...
import { supabaseDelete, supabaseGet, supabasePost } from "./supabaseRest";
...
export async function computePredictions(propertyId) {
  ...
  const upsert = await supabasePost("property_predictions", [predictions], "return=representation,resolution=merge-duplicates");
  ...
  await supabaseDelete(`property_prediction_reasons?property_id=eq.${propertyId}`);
