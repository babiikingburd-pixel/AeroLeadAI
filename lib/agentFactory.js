...
export async function create(spec) {
  ...
  const insertRes = await supabasePost("agent_specs", [{
    ...
    // expires_at is a plain column, not DB-generated (timestamptz + interval
    // isn't immutable), so it's computed here at insert time instead.
    expires_at: new Date(Date.now() + lifetimeHours * 3600 * 1000).toISOString(),
  }]);
