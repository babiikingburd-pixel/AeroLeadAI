...
export async function applyEvidenceAndRescore(supabase, leadRow, evidencePatch, changedBy, reasonMeta = {}) {
  ...
  const merged = {
    ...
    permitChecked,
    hailInches: evidencePatch.hail_inches ?? evidencePatch.hailInches ?? leadRow.hail_inches,
    windMph: evidencePatch.wind_mph ?? evidencePatch.windMph ?? leadRow.wind_mph,
    assessedValue: evidencePatch.assessed_value ?? evidencePatch.assessedValue ?? leadRow.assessed_value,
    ...
    drivewayCrackRisk: (evidencePatch.driveway_score ?? leadRow.driveway_score ?? 0) >= 50,
    gutterIndicator: evidencePatch.gutter_indicator ?? leadRow.damage_notes?.gutterIndicator === true,
    heavySnowRegion: evidencePatch.heavy_snow_region ?? leadRow.heavy_snow_region === true,
    heavyRainRegion: evidencePatch.heavy_rain_region ?? leadRow.heavy_rain_region === true,
  };
  ...
  const dbPatch = {
    ...
    evidence_score_breakdown: scored.breakdown ?? {},
    evidence_score_reasons: scored.reasons ?? [],
