...
export function scoreConfidence(lead, evidenceResult) {
  ...
  let points = 0;
  const checks = [
    lead.permit_within_10y !== undefined && lead.permit_within_10y !== null, // permit status actually checked, not just missing/unknown
...
export function calculatePriority(lead) {
  ...
  return { priorityScore, evidenceScore, confidenceScore, categories, breakdown, humanReview, entered: true, route };
}
