...
const REASON_LABELS = {
  maturity: "Property age",
  heavy_snow: "Heavy snow region",
  heavy_rain: "Heavy rain region",
  tree_overhang: "Tree overhang",
  large_overhang: "Large tree overhang",
  hail: "Hail exposure",
  wind: "Wind exposure",
  driveway: "Driveway cracking risk",
  gutter: "Gutter indicator",
};

// Turns the raw breakdown object into an explainable, ordered list —
// "why did this score 91" instead of just the number. Deliberately does NOT
// invent a per-signal confidence figure: this engine tracks confidence
// holistically (scoreConfidence), not per-signal, and fabricating
// per-reason confidence numbers would just be decoration, not evidence.
function buildReasons(breakdown, propertyFactor, jobEstimate, valueWeight = 0.35) {
  ...
  for (const [key, label] of Object.entries(REASON_LABELS)) {
    if (breakdown[key] != null && breakdown[key] !== 0) {
      reasons.push({ signal: key, label, contribution: breakdown[key] });
    }
  }
  if (propertyFactor > 0) {
    reasons.push({ signal: "property_value", label: "Assessed property value", contribution: Math.round(propertyFactor * 100 * valueWeight * 100) / 100 });
  }
  if (jobEstimate > 0) {
    reasons.push({ signal: "job_estimate", label: "Estimated job value", contribution: Math.round(jobEstimate * 0.0001 * 0.20 * 100) / 100 });
  }
  if (breakdown.county_multiplier != null && breakdown.county_multiplier !== 1.0) {
    reasons.push({ signal: "county_multiplier", label: "County multiplier", contribution: `×${breakdown.county_multiplier}` });
  }
...
export function calculatePriority(lead) {
  ...
  // ASSESSED VALUE WEIGHTING.
  //
  // This line is why the Top 500 came out as an assessed-value sort. Every
  // property 25+ years old receives an identical 60 maturity points, so among
  // mature homes the ONLY differentiator left is value:
  //
  //   1948 house, $1,199,100 -> 60*0.45 + 1.199*100*0.35 = 68.97 -> x1.30 = 89.66
  //
  // Defaults below reproduce the existing behaviour EXACTLY (cap 2.0, weight
  // 0.35) — nothing changes until you set the env vars. Lower the cap to stop
  // million-dollar properties from buying rank, or lower the weight to make
  // evidence matter more relative to value. Tune after storm backfill has run,
  // not before: right now value is the only populated signal, so damping it
  // would just flatten the ranking into noise.
  ...
  const raw =
    evidenceScore * 0.45 +
    propertyFactor * 100 * valueWeight +
