...
function buildReasons(breakdown, propertyFactor, jobEstimate) {
  const reasons = [];
  ...
  if (propertyFactor > 0) {
    reasons.push({ signal: "property_value", label: "Assessed property value", contribution: Math.round(propertyFactor * 100 * 0.35 * 100) / 100 });
  }
  if (jobEstimate > 0) {
...
export function calculatePriority(lead) {
  ...
  const reasons = buildReasons(breakdown, propertyFactor, jobEstimate);
