...
import { calculateAeroLeadScore } from "./aeroleadScore";
...
export function scoreRow(row) {
  ...
  const sourceStatus = { permit: permitChecked, storm: stormChecked, assessor: assessorChecked, imagery: imageryChecked };
  const aeroLead = calculateAeroLeadScore({
    priorityScore: scored.priorityScore,
    evidenceConfidence: scored.confidenceScore,
    sourceStatus,
  });
  return {
    ...row,
    ...scored,
    sourceStatus,
    aeroLeadScore: aeroLead.score,
    aeroLeadScoreVersion: aeroLead.version,
    aeroLeadScoreBreakdown: aeroLead.components,
    aeroLeadMissingEvidence: aeroLead.missingEvidence,
  };
}
