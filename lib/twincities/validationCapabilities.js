 ...
 *   - stormRealtime: a SEPARATE free, keyless source (api.weather.gov's
 *     active alerts) that genuinely does answer one lat/lon on demand — see
 *     POST /api/twincities/free-evidence. Only covers CURRENT/recent
 *     weather, not deep historical hail swaths, so it complements the
 *     bulk-only `storm` entry above rather than replacing it.
...
export const VALIDATION_CAPABILITIES = {
  ...
  stormRealtime: {
    status: "working",
    dimension: "storm_evidence_status",
    checkedAtField: "storm_date",
    notes: "NWS active alerts (api.weather.gov) via POST /api/twincities/free-evidence — free, keyless, real per-property on-demand call. Only sees CURRENT/recent weather, not historical hail swaths, so it's additive alongside the bulk-only `storm` entry, not a replacement for it.",
  },
  value: {
    status: "broken",
