...
export async function planCycle(tier = "high", maxItems = 10) {
  ...
  for (const p of poolRes.data) {
    ...
    // keep this affordable at the "high" tier's daily cadence (Vercel Hobby plan caps cron jobs to once/day; would be more frequent on a paid plan).
    ...
    const computedPriority = priority({
      ...
      lastScannedAt: p.predicted_at, stormAlertActive: false, // live weather check is a real API call per property — deliberately NOT done for every candidate on every run; the medium/low tiers below do the storm-context work
      recentPermit,
    });
