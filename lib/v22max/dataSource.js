...
import {signImageRows} from "../imagery/privateStorage.mjs";
...
export async function getLeadImages(ids=[]){
  ...
  try{
    ...
    const signed=await signImageRows(c,data,900).catch(()=>data);
    return signed.reduce((acc,img)=>{const id=String(img.property_id||"");if(!id)return acc;(acc[id]||(acc[id]=[])).push({...img,image_url:img.signed_url||img.image_url||null});return acc;},{});
  }catch{return{};}
}

export function supabaseStatus(){const configured=Boolean(supabaseServer());return{configured,adminClient:configured,browserClient:false,mode:"service-role-only"}}
